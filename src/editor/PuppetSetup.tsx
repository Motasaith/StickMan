...
import { imageLookup } from "@/runtime/media";
...
export function PuppetSetup({ asset, onClose }: { asset: Asset; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="max-h-[92vh] w-[820px] max-w-full overflow-y-auto rounded-2xl border border-line bg-panel p-5 text-foreground shadow-2xl">
        <p className="font-display text-lg font-semibold">Make "{asset.name}" a character</p>
        ...
        <div className="mt-4 flex flex-wrap gap-4">
          <div className="relative flex-none rounded-md bg-white" style={{ width: asset.w * k, height: asset.h * k }}>
            <img src={asset.src} alt="" className="block h-full w-full rounded-md" draggable={false} />
            <svg
              ref={svgRef}
              width={asset.w * k}
              height={asset.h * k}
          ...
          <div className="flex min-w-[260px] flex-1 flex-col gap-3">
            <canvas ref={previewRef} width={360} height={300} className="w-full rounded-md bg-white" />
            <p className="text-xs text-muted-foreground">
              Preview: <span className="text-[#ff8a3d]">orange</span> dots are the left side of the picture, <span className="text-[#2ec4b6]">teal</span> the right.
            </p>
            {status && <p className="text-xs text-muted-foreground">{status}</p>}
            <div className="mt-auto flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={findJoints}>
                <Sparkles className="size-3.5" /> Find joints with AI
              </Button>
              <Button variant="outline" size="sm" onClick={async () => setJoints(fitTemplate(await silhouette(asset)))}>
                Fit dots to outline
              </Button>
